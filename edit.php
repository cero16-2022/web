...
// Default form data
$form_data = [
    'id' => '', 'nro_ticket' => '', 'cliente' => '', 'local' => '', 'descripcion' => '', 
    'fecha_solicitud' => '', 'fecha_ejecucion' => '', 
    'sub_total' => '', 'total' => '', 'estado' => '', 'nro_oc_os' => '', 
    'nro_certificacion' => '', 'fecha_sustentado' => '', 'nro_factura' => '', 'fecha_facturacion' => ''
];

// 1. Fetch existing service data if ID is provided in GET request
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    try {
        ...
        if ($service_to_edit) {
            $form_data = array_merge($form_data, $service_to_edit);
            // Ensure dates are in YYYY-MM-DD format for input type="date"
            $form_data['fecha_solicitud'] = $service_to_edit['fecha_solicitud'] ? date('Y-m-d', strtotime($service_to_edit['fecha_solicitud'])) : '';
            $form_data['fecha_ejecucion'] = $service_to_edit['fecha_ejecucion'] ? date('Y-m-d', strtotime($service_to_edit['fecha_ejecucion'])) : ''; 
            $form_data['fecha_sustentado'] = $service_to_edit['fecha_sustentado'] ? date('Y-m-d', strtotime($service_to_edit['fecha_sustentado'])) : '';
            $form_data['fecha_facturacion'] = $service_to_edit['fecha_facturacion'] ? date('Y-m-d', strtotime($service_to_edit['fecha_facturacion'])) : '';
        } else {
    ...
    $errors[] = "No se proporcionó un ID de servicio para editar.";
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $total = trim($form_data['total']);
    $fecha_ejecucion = trim($form_data['fecha_ejecucion']);

    // Validation
    if (empty($nro_ticket)) {
        $errors[] = "El N° de Ticket es obligatorio.";
    } else if ($nro_ticket !== 'No aplica') {
        // Check for uniqueness, excluding the current record
        try {
            $stmt = $pdo->prepare("SELECT id FROM servicios WHERE nro_ticket = ? AND id != ?");
            $stmt->execute([$nro_ticket, $id]);
            if ($stmt->fetch()) {
                $errors[] = "El N° de Ticket '".htmlspecialchars($nro_ticket)."' ya existe en otro servicio. Debe ser único.";
            }
        } catch (PDOException $e) {
            $errors[] = "Error al verificar el N° de Ticket: " . $e->getMessage();
        }
    }
    ...
    if (!is_numeric($total) && !empty($total)) $errors[] = "El total debe ser un número.";
    if (!is_numeric($form_data['sub_total']) && !empty($form_data['sub_total'])) $errors[] = "El subtotal debe ser un número.";

    if (empty($errors)) {
        try {
            $sub_total_for_db = (empty($form_data['sub_total']) || !is_numeric($form_data['sub_total'])) ? 0 : (float)$form_data['sub_total'];
            $igvRate = 0.18; // Consistent IGV rate
            $total_for_db = $sub_total_for_db * (1 + $igvRate);
            ...
            $sql = "UPDATE servicios SET 
                        ...
                        fecha_sustentado = :fecha_sustentado, 
            ...
            $stmt->execute([
                ':nro_ticket' => $nro_ticket,
                ':cliente' => $cliente,
                ':local' => empty($form_data['local']) ? null : $form_data['local'],
                ':descripcion' => $descripcion,
                ':fecha_solicitud' => $fecha_solicitud,
                ':fecha_ejecucion' => empty($form_data['fecha_ejecucion']) ? null : $form_data['fecha_ejecucion'],
                ':sub_total' => $sub_total_for_db,
                ':total' => $total_for_db,
                ':estado' => $form_data['estado'],
                ':nro_oc_os' => empty($form_data['nro_oc_os']) ? null : $form_data['nro_oc_os'],
                ':nro_certificacion' => empty($form_data['nro_certificacion']) ? null : $form_data['nro_certificacion'],
                ':fecha_sustentado' => empty($form_data['fecha_sustentado']) ? null : $form_data['fecha_sustentado'],
                ':nro_factura' => empty($form_data['nro_factura']) ? null : $form_data['nro_factura'],
                ':fecha_facturacion' => empty($form_data['fecha_facturacion']) ? null : $form_data['fecha_facturacion'],
                ':id' => $id
            ]);

            $success_message = "¡Servicio actualizado con éxito! <a href='index.php' class='alert-link'>Volver a la lista</a>";
            
        } catch (PDOException $e) {
            if ($e->errorInfo[1] == 1062) {
                 $errors[] = "Error: El N° de Ticket '".htmlspecialchars($nro_ticket)."' ya existe. Debe ser único.";
            } else {
...
<form action="edit.php" method="POST" novalidate>
    ...
    <div class="card form-card">
        ...
        <div class="card-footer form-footer text-end">
            <button type="submit" class="btn btn-primary btn-lg"><i class="bi bi-arrow-repeat me-2"></i>Actualizar Servicio</button>
